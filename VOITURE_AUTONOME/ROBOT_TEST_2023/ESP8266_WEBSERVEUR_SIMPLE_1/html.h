...
<html lang="fr">
    ...
    <body>
      <div id="map">
          <div id="vehicle"></div>
          <div id="arrow"></div> <!-- Ajoutez cette ligne pour la flèche -->
    ...
    <script>
      ...
      function moveVehicle(instruction) {
          ...
          adjustMapSize(Math.max(currentPosition.x + vehicleWidth, window.innerWidth), Math.max(currentPosition.y + vehicleHeight, window.innerHeight));

          // Met à jour la position et la rotation de la flèche
          arrow.style.transform = 'rotate(' + (90 * direction) + 'deg)';
          arrow.style.left = (currentPosition.x + vehicleWidth / 2 - 5) + 'px';
          arrow.style.top = (currentPosition.y + vehicleHeight / 2 - 20) + 'px';

          // Met à jour la position du véhicule
          vehicle.style.left = currentPosition.x + 'px';
          vehicle.style.top = currentPosition.y + 'px';

          instructionDisplay.textContent = 'Déplacement en cours: ' + instruction;
      }
